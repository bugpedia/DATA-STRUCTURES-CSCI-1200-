...
void StudentTests() {
  ...
  //Case with 3 engines, splits it up so that speeds are as equal as possible
  t = NULL;
  PushBack(t, TrainCar::MakeEngine()); 
  PushBack(t, TrainCar::MakeEngine());
  PushBack(t, TrainCar::MakePassengerCar());
  ...
  PushBack(t, TrainCar::MakePassengerCar());
  PushBack(t, TrainCar::MakeDiningCar());
  SeparateTestHelper(t, "Student #1");

  //Case with 4 engines, keeps total cost low
  t = NULL;
  PushBack(t, TrainCar::MakeEngine()); 
  PushBack(t, TrainCar::MakeEngine());
  PushBack(t, TrainCar::MakePassengerCar());
  ...
  PushBack(t, TrainCar::MakeSleepingCar());
  PushBack(t, TrainCar::MakePassengerCar());
  SeparateTestHelper(t, "Student #2");

  //Testing ShipFreight
  std::cout << "============================================================================" << std::endl;
  std::cout << "SHIP FREIGHT TEST 1" << std::endl;
  MTRand_int32 mtrand_test1(42);
  //Uses all freight, and distributes weight as evenly as possible, keeps speeds
  ShipFreightHelper(mtrand_test1,10,10,70,4);

  MTRand_int32 mtrand_test2(42);
  std::cout << "SHIP FREIGHT TEST 2" << std::endl;
  ShipFreightHelper(mtrand_test2,6,5,100,2);
