...
void additional_student_tests() {
  ...
  std::cout << "Board created, width of 7." << std::endl;
  Tetris tetris(7);
  tetris.print();

  std::cout << "Two pieces added, testing the fact that L comes down correctly" << std::endl;
  tetris.add_piece('I',90,1);
  tetris.add_piece('L',90,0);
  tetris.print();

  std::cout << "Adding a Z piece rotated 180, we can fill up the last row." << std::endl;
  tetris.add_piece('Z',180,4);
  tetris.print();

  std::cout << "Scoring the first row..." << std::endl;
  int score = tetris.remove_full_rows();
  tetris.print();
  assert (score == 1);

  std::cout << "Adding more pieces to make sure they fall down correctly" << std::endl;
  tetris.add_piece('T',270,3);
  tetris.add_piece('I',0,6);
  tetris.add_piece('L',180,3);
  tetris.add_piece('O',0,1);
  tetris.add_piece('J',180,0);
  tetris.print();

  std::cout << "If one more piece is added, we can score three rows!" << std::endl;
  tetris.add_piece('I',180,5);
  tetris.print();

  std::cout << "Now to score them..." << std::endl;

  score += tetris.remove_full_rows();
  assert (score == 4);
  tetris.print();

  std::cout << "Adding one more piece just to ensure it works right" << std::endl;
  tetris.add_piece('T',90,5);
  tetris.print(); 

  std::cout << "And it does, now to destroy all dynamically allocated data." << std::endl;
  tetris.destroy();
