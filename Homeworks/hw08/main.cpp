...
void student_tests() {
  ...
  // First test of a QuadTree with double and char, so that we can still use 
  // plot to check

  vector< std::pair<Point<double>,char> > test_points;
  test_points.push_back(make_pair(Point<double>(15.5,8.5), 'A'));
  test_points.push_back(make_pair(Point<double>(10.5,5.0), 'B'));
  test_points.push_back(make_pair(Point<double>(2.5,4.9), 'C'));
  test_points.push_back(make_pair(Point<double>(11.35,18.3), 'D'));
  test_points.push_back(make_pair(Point<double>(28.9,23.4), 'E'));

  QuadTree<double,char> test;

  for (int i = 0; i < test_points.size(); i++) 
  {
    test.insert(test_points[i].first,test_points[i].second);
  }

  cout << "Plot of QuadTree<double,char> test" << endl;
  test.plot(40,30);
  cout << endl;

  // --------------------------------------------------------------------------

  // Replacing the char labels in the above test to std::string, and using 
  // print_sideways() to show that they were inserted correctly, since
  // operator< is defined for std::string
  vector< std::pair<Point<double>,string> > test_points2;
  test_points2.push_back(make_pair(Point<double>(15.5,8.5), "Advil"));
  test_points2.push_back(make_pair(Point<double>(10.5,5.0), "Box"));
  test_points2.push_back(make_pair(Point<double>(2.5,4.9), "Class"));
  test_points2.push_back(make_pair(Point<double>(11.35,18.3), "Door"));
  test_points2.push_back(make_pair(Point<double>(28.9,23.4), "Eagle"));

  QuadTree<double,string> test2;

  for (int i = 0; i < test_points2.size(); i++) 
  {
    test2.insert(test_points2[i].first,test_points2[i].second);
  }

  cout << "Sideways print of QuadTree<double,string> test2" << endl;
  test2.print_sideways();

  cout << endl;
  cout << "Now to insert a couple more points to test2" << endl;

  pair<QuadTree<double,string>::iterator,bool> ret_val;
  ret_val = test2.insert(Point<double>(39.9, 29.9), "Felix");

  cout << endl;
  if (ret_val.second)
    cout << "Point<double>(39.9, 29.9), ''Felix'' was inserted" << endl;
  test2.print_sideways();

  // Test of iteration. "itr" is pointing to the last element of the list
  // therefore calling ++ should make the itr go to the end and become NULL
  QuadTree<double,string>::iterator itr = ret_val.first;
  itr++;
  assert(itr == NULL);

  // Test of breadth first iteration specifically, and making sure it is
  // printing in the right order
  cout << "Testing breadth-first iteration." << endl;
  int level = -1;
  QuadTree<double,string>::bf_iterator bf_itr = test2.bf_begin();
  for (int i = 0; i < 6; i++) {
    assert (bf_itr != test2.bf_end());
    int depth = bf_itr.getDepth();
    if (level != depth) {
      level = depth;
      std::cout << std::endl << "   level " << level << ":";
  ...
  // Testing corner cases for insert, the copy constructor, assignment 
  // operator, and the destructor
  cout << endl;
  cout << "Dynamically create a new empty tree, 'a_tree'" << endl;
  QuadTree<float,int>* a_tree = new QuadTree<float,int>;
  a_tree->insert( Point<float>(10.0,5.78), 1);
  a_tree->insert( Point<float>(10.0,5.78), 1);
  cout << "Attempt to insert same value twice, however it doesn't insert twice"
       << endl;

  a_tree->print_sideways();

  // Copy constructor
  QuadTree<float,int> other_tree (*a_tree);
  other_tree.insert( Point<float>(87.3,98.0), 2);
  other_tree.insert( Point<float>(32.345,20.4), -9);

  cout << endl;
  cout << "After using the copy constructor to create another tree, 'other_tree'" << endl;
  cout << "\n\n'a_tree'" << endl;
  a_tree->print_sideways();
  cout << "\nother_tree" << endl;
  other_tree.print_sideways();

  QuadTree<float,int> tree_three;
  tree_three.insert(Point<float>(76.2,93.1), 29);
  tree_three.insert(Point<float>(76.3,93.9), 59);
  tree_three.insert(Point<float>(76.143,93.2), 94);
  tree_three.insert(Point<float>(76.1,93.05), 76);

  cout << "\nA new tree, 'tree_three', is initialized" << endl;
  tree_three.print_sideways();

  other_tree = tree_three;
  cout << "\nAfter calling the assignment operator, other_tree is the same as tree_three" << endl;
  cout << "\n'other_tree'" << endl;
  other_tree.print_sideways();
  cout << "\n'tree_three'" << endl;
  tree_three.print_sideways();

  tree_three.insert(Point<float>(8.2,39.2), 9000);

  cout << "\nOnly tree_three changes after an insert is made to tree_three" << endl;
  cout << "\n'other_tree'" << endl;
  other_tree.print_sideways();
  cout << "\n'tree_three'" << endl;
  tree_three.print_sideways();
  ...
  // Another test of iterators, making sure find works properly and returns the
  // correct iterator, and 
  QuadTree<float,int>::iterator df_itr = other_tree.find(76.2,93.1);
  assert (df_itr == other_tree.begin());
  df_itr++;
  assert (df_itr != other_tree.begin());

  // Testing the destructor. When ran with Dr. Memory, no leaks occur, so we're
  // done! Any non-dynamically allocated QuadTrees will have the desctructor
  // called for them once they leave the scope of the function.
  delete a_tree;
  a_tree = NULL;
