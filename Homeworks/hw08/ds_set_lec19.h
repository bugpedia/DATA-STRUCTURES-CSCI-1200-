// Partial implementation of binary-tree based set class similar 
// to std::set.  
...
public:
  ...
  TreeNode(const T& 
    init) : value(init), left(NULL), right(NULL), parent(NULL) {}
  T value;
  TreeNode* left;
  TreeNode* right;
  TreeNode* parent; 
...
public:
  ...
  bool operator== (const tree_iterator& rgt) 
  { return (set_ == rgt.set_ && ptr_ == rgt.ptr_); }
  bool operator!= (const tree_iterator& rgt) 
  { return (set_ != rgt.set_ || ptr_ != rgt.ptr_); }
...
public:
  ...
  bool operator==(const ds_set<T>& old) const 
...
private:
  ...
  void print_as_sideways_tree(std::ostream& ostr, 
    const TreeNode<T>* p, int depth) const {
    if (p) {
